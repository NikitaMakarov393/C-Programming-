...
int main(){
    ...
    int pd = 1;
    int gd = 1;
    printf("Введите колличество неизвестных СЛАУ: ");
    scanf("%d", &n);
    int MATRIX[n][n+1];
    int R[n][1];
    int G[n][n];
    int XYZ[n][n];
    printf("Введите данные для СЛАУ: / ");
    for(int i = 0; i < n; i++){
        for( int j =0; j < n+1; j++){
            scanf("%d", &MATRIX[i][j]);
        }
    ...
    if (n == 2){
            opr0 = G[0][0]*G[1][1]-G[0][1]*G[1][0];                 //Вычисление определителя системы opr0 для 2 СЛАУ с 2 неизвестными
    }
    else{
        for(int q = 0; q < n;q++){   
                        gd = 1;
